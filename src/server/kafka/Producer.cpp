...
Producer::Producer(string broker):brokerIp_(broker){
    ...
    if(conf_->set("bootstrap.servers",brokerIp_,errstr_) !=  RdKafka::Conf::CONF_OK){
        ERROR_LOG("Producer conf create error : %s",errstr_.c_str());
        exit(1);
    }
    ...
    if(!producer_){
        ERROR_LOG("producer create error : %s",errstr_.c_str());
        exit(1);
    }
...
Producer::~Producer(){
    ...
    {
        ERROR_LOG(" waitint for: : %d",producer_->outq_len());
...
bool Producer::produce(const string& topicName,const string &messages){
    ...
    if(err != RdKafka::ERR_NO_ERROR){
        ERROR_LOG("topic: %s produce error:%s",topicName.c_str(),RdKafka::err2str(err).c_str());
